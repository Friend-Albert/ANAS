...
class DeviceDataset(Dataset):
    def __init__(self, input_file: list, seq_len: int, use_gpu: bool = True, input_dim: int = None, verbose=False,
                 item_to_cuda: bool = False):
    ...
    def test_run(self, file_idx=0):
        with open(self.input_file[file_idx], "rb") as f:
            one_data = np.load(f)
            assert self.input_dim + 1 == len(
                one_data[0]), f"Input dim value: {self.input_dim}, data feature num: {len(one_data[0])}"
        ...
        for data_idx, (start_idx, one_data_len) in enumerate(zip(self.data_start_idx, self.data_len)):
            if (idx_in >= start_idx) and (idx_in < start_idx + one_data_len):
                ...
                if self.item_to_cuda:
                    return self.data[data_idx][idx: idx + self.seq_len, : self.input_dim].cuda(), self.data[data_idx][
                        idx + self.seq_len - 1, -1].cuda()
                else:
                    return self.data[data_idx][idx: idx + self.seq_len, : self.input_dim], self.data[data_idx][
                        idx + self.seq_len - 1, -1]
...
class PreporcessDataset:
    ...
    def sample_validation_data(self):
        print('start merge_and_normalize')
        self.merge_and_normalize()
        print('end merge_and_normalize')
        ...
        print('end sample_validation_data')
